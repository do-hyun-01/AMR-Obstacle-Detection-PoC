...
def run_process():
    ...
    for root, dirs, files in os.walk(IMG_ROOT):
        if os.path.basename(root) == "images":
            # images 폴더의 부모 폴더 이름 (예: S_I_001) 추출 후 키로 사용
            seq_name = os.path.basename(os.path.dirname(root))
            img_folder_map[seq_name] = root

    print(f" JSON 분석 및 매칭 (대상 : {len(img_folder_map)}개)")
    all_data_tasks = []
    ...
    for root, dirs, files in os.walk(LBL_ROOT):
        for file in files:
            ...
            # 경로에서 S_I_xxx 추출 (리스트 인덱싱으로 단순화)
            parts = json_path.split(os.sep)
            seq_name = ""
            for p in parts:
                if p.startswith("S_I_"): seq_name = p

            if not seq_name or seq_name not in img_folder_map: continue
